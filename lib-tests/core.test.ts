...
import PrivateCoachingAmm, { DEFAULT_PROGRAM_ID, isAddress } from '../app'
...
describe('core', function () {
  ...
  let privateCoachingAmm: PrivateCoachingAmm,
  ...
  before(async () => {
    const { program } = new PrivateCoachingAmm(wallet)
  ...
  it('constructor', async () => {
    privateCoachingAmm = new PrivateCoachingAmm(wallet)
    if (privateCoachingAmm.program.programId.toBase58() !== DEFAULT_PROGRAM_ID)
      throw new Error('Cannot contruct a privateCoachingAmm instance')
    // Setup test supporters
    connection = privateCoachingAmm.program.provider.connection
  })

  it('create pool', async () => {
    const { poolAddress: _poolAddress } = await privateCoachingAmm.createPool({
  ...
  it('Get data #1', async () => {
    const { x, y } = await privateCoachingAmm.getPoolData(poolAddress)
    expect(x.eq(new BN('500000000000'))).to.be.true
    expect(y.eq(new BN('500000000000'))).to.be.true
  })

  it('swap', async () => {
    const { txId } = await privateCoachingAmm.swap({
      a: new BN('5000000000'),
      poolAddress,
    })
